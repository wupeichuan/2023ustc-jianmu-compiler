...
void Value::add_use(User *user, unsigned arg_no) { use_list_.emplace_back(user, arg_no); };
...
void Value::replace_use_with_if(Value *new_val, std::function<bool(Use)> should_replace) {
  if (this == new_val) return;
  for (auto iter = use_list_.begin(); iter != use_list_.end();) {
    auto use = *iter++;
    if (not should_replace(use)) continue;
    use.val_->set_operand(use.arg_no_, new_val);
  }
}
