...
ASTNode *AST::transform_node_iter(syntax_tree_node *n) {
  if (_STR_EQ(n->name, "program")) {
    ...
    if (_STR_EQ(n->children[0]->children[0]->name, "int"))
      ...
      node->type = TYPE_FLOAT;
    node->id = n->children[1]->name;
    if (n->children_num > 2) node->isarray = true;
    return node;
...
Value *ASTPrinter::visit(ASTSelectionStmt &node) {
  ...
  if (node.else_statement != nullptr) node.else_statement->accept(*this);
  remove_depth();
  return nullptr;
}
...
Value *ASTPrinter::visit(ASTSimpleExpression &node) {
  ...
  if (node.additive_expression_r != nullptr) node.additive_expression_r->accept(*this);
  remove_depth();
  return nullptr;
}
...
Value *ASTPrinter::visit(ASTAdditiveExpression &node) {
  ...
  add_depth();
  if (node.additive_expression != nullptr) node.additive_expression->accept(*this);
...
Value *ASTPrinter::visit(ASTTerm &node) {
  ...
  add_depth();
  if (node.term != nullptr) node.term->accept(*this);
