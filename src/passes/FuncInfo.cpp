...
void FuncInfo::run() {
  for (auto &f : m_->get_functions()) {
    auto func = &f;
    trivial_mark(func);
    if (not is_pure[func]) worklist.push_back(func);
  }
...
bool FuncInfo::is_side_effect_inst(Instruction *inst) {
  if (inst->is_store()) {
    if (is_local_store(dynamic_cast<StoreInst *>(inst))) return false;
    return true;
  }
  if (inst->is_load()) {
    if (is_local_load(dynamic_cast<LoadInst *>(inst))) return false;
    return true;
  }
...
bool FuncInfo::is_local_load(LoadInst *inst) {
  ...
  if (addr and addr->is_alloca()) return true;
  return false;
}
...
Value *FuncInfo::get_first_addr(Value *val) {
  if (auto inst = dynamic_cast<Instruction *>(val)) {
    if (inst->is_alloca()) return inst;
    if (inst->is_gep()) return get_first_addr(inst->get_operand(0));
    if (inst->is_load()) return val;
    LOG_WARNING << "FuncInfo: try to determine addr in operands";
    for (auto op : inst->get_operands()) {
      if (op->get_type()->is_pointer_type()) return get_first_addr(op);
    }
  }
  return val;
}
