...
void RegAlloca::live_interval(Function *f) {
  ...
  for (auto bb : postorder) {
    ...
    for (auto val : activevar_->get_out(bb)) add_range(val, bb_from, bb_to);
  }
  ...
  for (auto bb : r_postorder) {
    ...
    while (iter != bb->get_instructions().begin()) {
      ...
      if (ir->is_add() || ir->is_sub() || ir->is_mul() || ir->is_div() || ir->is_fadd() || ir->is_fsub() ||
        ...
        if (!dynamic_cast<GlobalVariable *>(ir)) add_range(ir, inst_id[ir] + 1, -1);
      } else if (ir->is_store()) {
        ...
        if (!ir->is_void() && !dynamic_cast<GlobalVariable *>(ir)) {
          ...
          if (ir->get_name() != "neg_idx_except") add_range(ir, inst_id[ir] + 1, inst_id[ir] + 2);
        }
      } else if (ir->is_ret()) {
        if (!ir->is_void()) {
        ...
        if (!dynamic_cast<GlobalVariable *>(ir)) add_range(ir, inst_id[&(bb->get_instructions().front())], -1);
      } else if (ir->is_gep()) {
        for (auto val : ir->get_operands()) {
          if (!dynamic_cast<ConstantInt *>(val) && !dynamic_cast<ConstantFP *>(val) &&
              !dynamic_cast<GlobalVariable *>(val)) {
...
void RegAlloca::add_range(Value *val, int start, int end) {
  if (start == -1) {
    if (interval_[val].end == -1 || interval_[val].end < end) interval_[val].end = end;
  } else if (end == -1) {
    interval_[val].start = start;
  } else {
    if (interval_[val].start == -1) interval_[val].start = start;
    if (interval_[val].end == -1 || interval_[val].end < end) interval_[val].end = end;
  }
}
