...
void ActiveVar::create_activevar(Function *f) {
  ...
  for (auto &bb1 : f->get_basic_blocks()) {
    for (auto &inst : (&bb1)->get_instructions()) {
      auto ir = &inst;
      if (ir->is_add() || ir->is_sub() || ir->is_mul() || ir->is_div() || ir->is_fadd() || ir->is_fsub() ||
          ir->is_fmul() || ir->is_fdiv() || ir->is_cmp() || ir->is_fcmp() || ir->is_fp2si() || ir->is_si2fp() ||
          ir->is_zext()) {
        for (auto val : ir->get_operands()) {
          if (!dynamic_cast<ConstantInt *>(val) && !dynamic_cast<ConstantFP *>(val) &&
              !dynamic_cast<GlobalVariable *>(val)) {
            if (use_bb[&bb1].count(val) == 0 && def_bb[&bb1].count(val) == 0) use_bb[&bb1].insert(val);
          }
        }
        ...
        if (!dynamic_cast<ConstantInt *>(val_1) && !dynamic_cast<ConstantFP *>(val_1) &&
          ...
          if (use_bb[&bb1].count(val_1) == 0 && def_bb[&bb1].count(val_1) == 0) use_bb[&bb1].insert(val_1);
        }
        if (use_bb[&bb1].count(val_2) == 0 && def_bb[&bb1].count(val_2) == 0) use_bb[&bb1].insert(val_2);
      } else if (ir->is_load()) {
        auto val_1 = ir->get_operand(0);
        if (!dynamic_cast<ConstantInt *>(val_1) && !dynamic_cast<ConstantFP *>(val_1) &&
            !dynamic_cast<GlobalVariable *>(val_1)) {
  ...
  while (changed) {
    ...
    for (auto &bb1 : f->get_basic_blocks()) {
      ...
      for (auto succ : (&bb1)->get_succ_basic_blocks()) {
        std::set_union(live_in[succ].begin(), live_in[succ].end(), succ_union.begin(), succ_union.end(),
                       std::inserter(succ_union, succ_union.end()));
      }
      live_out[&bb1] = succ_union;
      std::set<Value *> diff;
      std::set_difference(live_out[&bb1].begin(), live_out[&bb1].end(), def_bb[&bb1].begin(), def_bb[&bb1].end(),
                          std::inserter(diff, diff.end()));
      auto record = live_in[&bb1];
      std::set_union(use_bb[&bb1].begin(), use_bb[&bb1].end(), diff.begin(), diff.end(),
                     std::inserter(live_in[&bb1], live_in[&bb1].end()));
      if (record != live_in[&bb1]) changed = true;
    }
  }
}