...
Value *CminusfBuilder::visit(ASTSelectionStmt &node) {
  ...
  if (node.else_statement != nullptr) {
    ...
    if (builder->get_insert_block()->is_terminated() == false) builder->create_br(brBB);
    scope.exit();
...
Value *CminusfBuilder::visit(ASTAssignExpression &node) {
  ...
  auto expressionLoad = node.expression->accept(*this);
  if (varAlloca->get_type()->get_pointer_element_type()->is_float_type() &&
      expressionLoad->get_type()->is_integer_type())
    expressionLoad = builder->create_sitofp(expressionLoad, FLOAT_T);
  else if (varAlloca->get_type()->get_pointer_element_type()->is_integer_type() &&
           expressionLoad->get_type()->is_float_type()) {
    if (varAlloca->get_type()->get_pointer_element_type()->is_int1_type())
      expressionLoad = builder->create_fcmp_ne(expressionLoad, ConstantFP::get(0, module.get()));
    else
      expressionLoad = builder->create_fptosi(expressionLoad, INT32_T);
  } else if (varAlloca->get_type()->get_pointer_element_type()->is_integer_type() &&
             expressionLoad->get_type()->is_integer_type()) {
    if (varAlloca->get_type()->get_pointer_element_type()->is_int1_type() &&
        expressionLoad->get_type()->is_int32_type())
      expressionLoad = builder->create_icmp_ne(expressionLoad, ConstantInt::get(0, module.get()));
    else if (varAlloca->get_type()->get_pointer_element_type()->is_int32_type() &&
             expressionLoad->get_type()->is_int1_type())
      expressionLoad = builder->create_zext(expressionLoad, INT32_T);
  } else {
  }
