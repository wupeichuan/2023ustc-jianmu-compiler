...
class AllocaInst : public BaseInst<AllocaInst> {
 ...
 public:
  ...
  Type *get_alloca_type() const { return get_type()->get_pointer_element_type(); };
...
class PhiInst : public BaseInst<PhiInst> {
 ...
 private:
  PhiInst(Type *ty, std::vector<Value *> vals, std::vector<BasicBlock *> val_bbs, BasicBlock *bb);

 public:
  static PhiInst *create_phi(Type *ty, BasicBlock *bb, std::vector<Value *> vals = {},
